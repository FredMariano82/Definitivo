...
const values = results.data.map((row: any) => {
  ...
  const dataEmprestimo = row['Data Empréstimo'] && row['Data Empréstimo'] !== '' ? (new Date(row['Data Empréstimo']).toString() === 'Invalid Date' ? 'NULL' : `'${new Date(row['Data Empréstimo']).toISOString()}'`) : 'NULL';

  return `(${escape(row['Número'] || row['Numero'])}, ${escape((row['Modelo'] || 'prata').toLowerCase())}, ${escape(row['Local'] || 'Desconhecido')}, ${escape(statusFinal)}, ${escape(row['OBS'] || '')}, ${escape(row['Responsável'] || row['Responsavel'])}, ${escape(row['Setor'])}, ${escape(row['Operador'])}, ${dataEmprestimo})`;
});
...
console.log("SQL Final gerado com sucesso em C:/Users/central_seguranca/.gemini/antigravity/brain/34ebb1bb-d24e-4997-9932-477c22987cb0/setup_chaves_final.sql");
