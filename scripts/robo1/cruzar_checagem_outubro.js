...
const SOURCE_FILE = path.join(DIR, "checagens OUT 2025.xlsx");
...
async function mergeOctober() {
    console.log("🚀 Iniciando cruzamento de checagem (OUTUBRO APENAS)...");
    ...
    console.log("📡 Lendo base de checagens de Outubro...");
    ...
    dataSource.forEach(row => {
        // Colunas identificadas via comando anterior: NOME, EMPRESA
        const nome = row['NOME'];
    ...
    // Colunas: [0] nome, [1] RG, [6] EMPRESA, [7] checagem válida até
    const idxNome = headers.indexOf('nome');
    const idxEmpresa = headers.indexOf('EMPRESA '); // Note o espaço no final detectado antes
    ...
    try {
        ...
        console.log(`\n✨ SUCESSO! Arquivo mestre atualizado com dados de Outubro.`);
    } catch (e) {
        if (e.code === 'EBUSY') {
...
mergeOctober().catch(e => console.error("❌ Erro fatal:", e));
