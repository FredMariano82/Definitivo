...
const SOURCE_FILE = path.join(DIR, "checagens NOV 2025.xlsx");
...
async function mergeNovember() {
    console.log("🚀 Iniciando cruzamento de checagem (NOVEMBRO V2)...");

    if (!fs.existsSync(SOURCE_FILE) || !fs.existsSync(MASTER_FILE)) {
        console.error("❌ Arquivos necessários não encontrados.");
        console.log("Fonte:", SOURCE_FILE);
        console.log("Mestre:", MASTER_FILE);
        return;
    }

    // 1. Carregar Fonte e criar mapa
    console.log("📡 Lendo base de checagens de Novembro...");
    ...
    dataSource.forEach(row => {
        // Colunas identificadas: NOME, RG, EMPRESA
        const nome = row['NOME'];
    ...
    try {
        ...
        console.log(`\n✨ SUCESSO! Arquivo mestre atualizado com dados de Novembro.`);
    } catch (e) {
        if (e.code === 'EBUSY') {
