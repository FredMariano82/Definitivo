...
export default function Liberacoes() {
    ...
    .flatMap((solicitacao) =>
      solicitacao.prestadores
        ...
        .map((p: any) => ({
          solicitacao,
          prestador: p,
          statusLiberacao: getLiberacaoStatus(p, solicitacao.dataFinal)
        }))
    )
  ...
  const dadosPorStatus = dadosFiltradosTotal
    .filter((item) => {
      ...
      const statusMatch = filtroStatus === "todos" || item.statusLiberacao === filtroStatus
  ...
  // Os cards de status também devem reagir ao filtro de EMPRESA para serem precisos
  const statusCounts = {
    pendente: dadosFiltradosTotal.filter((item) => (filtroEmpresa === "todos" || item.solicitacao.empresa === filtroEmpresa) && item.statusLiberacao === "pendente").length,
    ok: dadosFiltradosTotal.filter((item) => (filtroEmpresa === "todos" || item.solicitacao.empresa === filtroEmpresa) && item.statusLiberacao === "ok").length,
    urgente: dadosFiltradosTotal.filter((item) => (filtroEmpresa === "todos" || item.solicitacao.empresa === filtroEmpresa) && item.statusLiberacao === "urgente").length,
    vencida: dadosFiltradosTotal.filter((item) => (filtroEmpresa === "todos" || item.solicitacao.empresa === filtroEmpresa) && item.statusLiberacao === "vencida").length,
    negada: dadosFiltradosTotal.filter((item) => (filtroEmpresa === "todos" || item.solicitacao.empresa === filtroEmpresa) && item.statusLiberacao === "negada").length,
  }
            ...
            <div className="space-y-4">
              <div className="relative">
                <Input
                  ...
                  className="h-12 border-slate-200 focus:border-emerald-500 focus:ring-emerald-500 rounded-xl pr-12 text-slate-700 bg-slate-50/50 shelf-inner"
                />
              ...
              {mostrarFiltros && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 mt-4 border-t border-slate-100 animate-in fade-in slide-in-from-top-2 duration-300">
                  ...
                  {/* Filtro Status */}
                  <div className="space-y-2">
                    <Label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest pl-1">Por Liberação</Label>
                    <Select value={filtroStatus} onValueChange={setFiltroStatus}>
                      ...
                      <SelectContent className="rounded-xl border-slate-200 shadow-xl">
                        ...
                        <SelectItem value="ok">Ok / Liberado</SelectItem>
                        <SelectItem value="urgente">Urgente</SelectItem>
                        <SelectItem value="negada">Negado</SelectItem>
