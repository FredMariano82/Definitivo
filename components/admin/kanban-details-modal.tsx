        ...
        <ScrollArea className="flex-1 p-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-2 space-y-6">
              ...
              {tarefa.categoria === 'ronda_dvr' && (
                <section className="w-full">
                    <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-2">Miniatura do Relatório</h3>
                    {tarefa.dados_especificos?.relatorio_html_snapshot ? (
                        <div className="w-full bg-muted/30 rounded-xl border p-4 flex justify-center overflow-hidden" style={{ height: '640px' }}>
                            <div style={{ transform: 'scale(0.55)', transformOrigin: 'top center', width: '210mm', height: '297mm' }}>
                                <iframe 
                                    srcDoc={tarefa.dados_especificos.relatorio_html_snapshot}
                                    className="w-[210mm] h-[297mm] bg-white shadow-2xl border"
                                    title="Relatório Ronda CFTV"
                                />
                            </div>
                        </div>
                    ) : (
                        <div className="bg-muted p-8 rounded-lg text-center text-sm text-muted-foreground italic border border-dashed">
                            Relatório visual não foi salvo nesta tarefa legada.
                        </div>
                    )}
                </section>
              )}
